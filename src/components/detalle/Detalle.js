...
const Detalle = ({
  ...
  const valorTotal = formatoPesos(selectEvent.valorNoche*selectEvent.cantDias);
  const descuento = formatoPesos(calcularDescuento(selectEvent.descuento, selectEvent.cantDias, selectEvent.valorNoche));
  ...
  return (
    <Card className="containerDetalle">
      {edit ? (
        ...
        />
      ) : Object.keys(selectEvent).length === 0 ? (
        <Typography
          sx={{
            fontSize: 20,
            fontWeight: 'bold',
        ...
        <CardContent className="containerInfo">
          ...
          <div className="costosContainer">
            <div className='constoTotal'>
              <Typography sx={{ fontSize: 13 }}>Valor x días:</Typography>
              <Typography sx={{ fontSize: 13, fontWeight: 'bold' }}>
                {valorTotal === '$NaN' ? '$0' : selectEvent.cantDias + ' días = ' + valorTotal}
              </Typography>
            </div>
            <div className='constoTotal'>
              <Typography sx={{ fontSize: 13 }}>-20%:</Typography>
              <Typography sx={{ fontSize: 13, fontWeight: 'bold' }}>
                -{descuento === '$NaN' ? '$0' : descuento}
              </Typography>
            </div>
            <hr style={{backgroundColor:'red'}}/>
            <div className='constoTotal'>
              <Typography sx={{ fontSize: 15, fontWeight: 'bold' }}>Valor Total:</Typography>
              <Typography sx={{ fontSize: 15, fontWeight: 'bold' }}>
                {costoTotalArriendo === '$NaN' ? '$0' : costoTotalArriendo}
              </Typography>
            </div>
          </div>
          ...
          <div className="containerInfoContacto">
            ...
            <div className="detalleInfoContacto">
              <Typography sx={{ fontSize: 12, display: 'flex', gap: 0.5 }}>
                <MailIcon fontSize="small" sx={{ fontSize: 15 }} />
                {selectEvent.correo}
              </Typography>
              <Typography sx={{ fontSize: 12, display: 'flex', gap: 0.5 }}>
                <PhoneIcon fontSize="small" sx={{ fontSize: 15 }} />
                +56 9 {selectEvent.celular}
              </Typography>
              <Typography sx={{ fontSize: 12, display: 'flex', gap: 0.5 }}>
                <LocationOnIcon fontSize="small" sx={{ fontSize: 15 }} />
                {selectEvent.ubicacion}
              </Typography>
              <Button
                variant="contained"
                size="small"
                ...
                startIcon={<WhatsApp />}
                onClick={() =>
                  enviarMensajeWhatsapp(`569${selectEvent.celular}`)
                }
              >
                Whatsapp
              </Button>
